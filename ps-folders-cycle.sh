...
if [[ -f "$CONFIG" ]]; then
    ...
    notify-send "Config file ($CONFIG) not found. Using default configuration.";
    echo "Config file ($CONFIG) not found. Using default configuration.";
fi
...
Shuffle: $shuffle
Shuffle start: $shuffleStart
Startup notification: $startupNotif
Color change notification: $colorNotif
"

if [ $interval -lt 10 ]; then
    notify-send "You set an interval of $interval seconds? No way that's stable! We'll use 10 seconds. Ideal is 60 seconds or higher."
    interval=10
fi
colorsLength=${#colors[@]}
actualIndex=0
